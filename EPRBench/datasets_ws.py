...
class BaseDataset(data.Dataset):
    ...
    def __init__(self, args, datasets_folder="datasets", dataset_name="pitts30k", split="train"):
        ...
        if not os.path.exists(self.dataset_folder):
            # Fallback to direct path without "images" folder
        ...
        if args.use_text:
             # Assuming text files are in a 'text_descriptions' folder parallel to 'images' or specified by args.text_folder
            text_folder = args.text_folder if args.text_folder else join(datasets_folder, dataset_name, "text_descriptions")
            
            # If explicit folder not found, try to look for per-image txt files or a mapping
            # For simplicity, let's assume we might find .txt files with same basename as images
            # Or we just use a default placeholder if not found, to avoid crashing.
            # You can implement specific loading logic here.
            pass
        ...
        if self.args.use_text:
            ...
            if os.path.exists(txt_path):
                try:
                    ...
                    pass
            # Also check in a specific text folder if needed
            # ...
        
        return img, index, text
