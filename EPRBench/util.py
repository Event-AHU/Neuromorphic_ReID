...
def resume_model(args, model):
    ...
    if any(k.startswith("encoder_cross.") for k in state_dict.keys()) and not any(k.startswith("encoder_cross.") for k in model_keys):
        state_dict = OrderedDict({(k.replace("encoder_cross.", "encoder.", 1) if k.startswith("encoder_cross.") else k): v for (k, v) in state_dict.items()})
    if any(k.startswith("encoder.") for k in state_dict.keys()) and not any(k.startswith("encoder.") for k in model_keys):
        state_dict = OrderedDict({(k.replace("encoder.", "encoder_cross.", 1) if k.startswith("encoder.") else k): v for (k, v) in state_dict.items()})
    missing, unexpected = model.load_state_dict(state_dict, strict=False)
    if missing:
        logging.info(f"Missing keys when loading checkpoint (ignored): {len(missing)}")
    if unexpected:
        logging.info(f"Unexpected keys when loading checkpoint (ignored): {len(unexpected)}")
    return model
