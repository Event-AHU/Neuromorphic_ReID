...
def test_efficient_ram_usage(args, eval_ds, model, test_method="hard_resize", pca=None, text_encoder=None):
...
def test(args, eval_ds, model, test_method="hard_resize", pca=None, text_encoder=None):
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(database_dataloader, ncols=100):
            inputs, indices, text_desc = batch
            
            if text_desc is not None:
                assert text_encoder is not None, "Dataset provides text but no text_encoder passed"
                text_list = list(text_desc)  # 长度 = bs
                text_vec = text_encoder(text_list)                              # (bs, 768)
                text_tokens, attn_mask = text_encoder.encode_batch_tokens(text_list)  # (bs, L, 768), (bs, L)
                features = model(inputs.to(args.device), text=text_vec, text_tokens=text_tokens, text_mask=attn_mask)
            else:
                features = model(inputs.to(args.device))
        ...
        for batch in tqdm(queries_dataloader, ncols=100):
            ...
            if test_method == "five_crops" or test_method == "nearest_crop" or test_method == 'maj_voting':
                ...
                if text_vec is not None:
                    text_vec = text_vec.repeat_interleave(5, dim=0)  # shape = 5*bs x 768
                    text_tokens = text_tokens.repeat_interleave(5, dim=0)  # shape = 5*bs x L x 768
                    attn_mask = attn_mask.repeat_interleave(5, dim=0)  # shape = 5*bs x L
...
def top_n_voting(topn, predictions, distances, majority_weight):
    if topn == 'top1':
        n = 1
    elif topn == 'top5':
        n = 5
    ...
    for i in range(5):
        for j in range(n):
            ...
            count = 0
            for k in range(5):
                if pred in predictions[k, :n]:
                    count += 1
            if count > 1:
                distances[i, j] -= distances[i, j] * majority_weight * (count - 1)
