        ...
        if self.sample == 'random':
            ...
            for indices in indices_list:  # <class 'list'>: [0, 1, 2, 3, 4, 5, 6, 7]
                ...
                for index in indices:
                    ...
                    # img = img.unsqueeze(0)
                    imgs.append(img)
                    img_e = Image.open(img_path.replace('rgb_degrade', 'event').replace('rgb', 'event')).convert('RGB')
                    # img = img.unsqueeze(0)
                    imgs_e.append(img_e)
                ...
                imgs_e = torch.stack(imgs[1], 0)
                imgs = torch.stack(imgs[0], 0)  # torch.Size([8, 3, 224, 112])
                
                # torch.Size([8, 3, 224, 112])
            ...
            # flow_tensor = None
            return imgs_tensor, imgs_tensor_e, pid, camid, trackid, ""
            ...
            imgseq_e = [Image.open(img_path.replace('rgb_degrade', 'event').replace('rgb', 'event')).convert('RGB') for img_path in img_paths[number]]
            ...
            return  img_tensor, img_tensor_e, pid, camid, trackid, ""
