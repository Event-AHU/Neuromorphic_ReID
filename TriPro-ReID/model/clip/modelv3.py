...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, cv_emb = None, attr_embeds=None, visual_prompt=None, temporal_prompt=None):
        ...
        for bid, blk in enumerate(self.transformer.resblocks[:11]):
            ...
            # prompt switch
            if temporal_prompt is not None:
                temporal_prompt = x11[-tpl:].clone().contiguous()
                temporal_prompt = temporal_prompt.view(tpl, B//T, T, D).transpose(1,0)
                temporal_prompt = temporal_prompt.transpose(1,2) # 时序prompt交换帧信息
                temporal_prompt = temporal_prompt.transpose(1,0)
                temporal_prompt = temporal_prompt.contiguous().view(tpl, -1, D) 
                x11[-tpl:] = temporal_prompt
        if visual_prompt is not None:
