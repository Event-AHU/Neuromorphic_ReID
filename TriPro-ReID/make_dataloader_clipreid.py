...
from datasets.set.AER_rgb import AER_rgb
from datasets.set.AER_event import AER_event
from datasets.set.AER_rgb_degrade import AER_rgb_degrade
from datasets.set.AER_mix import AER_mix
__factory = {
	'mars': Mars,
...
def train_collate_fn(batch):
    ...
    """
	imgs, imgs_e, pids, camids, viewids, _ = zip(*batch)
	pids = torch.tensor(pids, dtype=torch.int64)
	viewids = torch.tensor(viewids, dtype=torch.int64)
	camids = torch.tensor(camids, dtype=torch.int64)
	return torch.stack(imgs, dim=0), torch.stack(imgs_e, dim=0), pids, camids, viewids,


def val_collate_fn(batch):
	imgs, imgs_e, pids, camids, viewids, img_paths = zip(*batch)
	viewids = torch.tensor(viewids, dtype=torch.int64)
	camids_batch = torch.tensor(camids, dtype=torch.int64)
	return torch.stack(imgs, dim=0),torch.stack(imgs_e, dim=0), pids, camids, camids_batch, viewids, img_paths
...
def train_collate_fn_seq(batch):
    ...
    # collate_fn这个函数的输入就是一个list，list的长度是一个batch size，list中的每个元素都是__getitem__得到的结果
    """
	imgs, imgs_e, flows, pids, camids = zip(*batch)
	viewids = 1
	pids = torch.tensor(pids, dtype=torch.int64)
	viewids = torch.tensor(viewids, dtype=torch.int64)
	camids = torch.tensor(camids, dtype=torch.int64)
...
def make_dataloader(cfg):
	...
	if cfg.DATASETS.NAMES != 'mars' and cfg.DATASETS.NAMES != 'duke' and cfg.DATASETS.NAMES != 'lsvid' and cfg.DATASETS.NAMES != 'rgb' and cfg.DATASETS.NAMES != 'event' and cfg.DATASETS.NAMES != 'rgb_degrade':
...
def make_eval_all_dataloader(cfg):
	...
	if cfg.DATASETS.NAMES != 'mars' and cfg.DATASETS.NAMES != 'duke' and cfg.DATASETS.NAMES != 'lsvid' and cfg.DATASETS.NAMES != 'rgb' and cfg.DATASETS.NAMES != 'event' and cfg.DATASETS.NAMES != 'mix' and cfg.DATASETS.NAMES != 'rgb_degrade':
...
def make_eval_rrs_dataloader(cfg):
	...
	if cfg.DATASETS.NAMES != 'mars' and cfg.DATASETS.NAMES != 'duke' and cfg.DATASETS.NAMES != 'lsvid' and cfg.DATASETS.NAMES != 'rgb' and cfg.DATASETS.NAMES != 'event' and cfg.DATASETS.NAMES != 'rgb_degrade' and cfg.DATASTES.NAMES != 'mix':
